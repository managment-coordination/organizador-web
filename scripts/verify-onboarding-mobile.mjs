...
try {
  ...
  for(const width of [390,360,1440]) {
    ...
    await page.locator('[data-onboarding-new-group]').click();
    await page.locator('#onboardingGroupForm [name="nombre"]').fill('General prueba '+suffix);
    const groupBounds=await page.locator('#onboardingGroupForm').boundingBox();
    assert.ok(groupBounds.x>=-1&&groupBounds.x+groupBounds.width<=width+1,'Inline group form clipped');
    await page.screenshot({path:path.join(output,`${width}-inline-group.png`)});
    const [groupResponse]=await Promise.all([page.waitForResponse(r=>r.url().endsWith('/api/erp/command')),page.locator('#onboardingGroupForm button[type="submit"]').click()]);
    assert.equal(groupResponse.status(),200,await groupResponse.text());
    const group=(await groupResponse.json()).entity;
    await page.locator('#onboardingGroupForm').waitFor({state:'detached'});
    assert.equal(await page.locator('.masterOnboardingMap').nth(2).inputValue(),'coeficiente_grupo:'+group.id_grupo);
    assert.equal(await page.locator('.masterOnboardingMap').nth(1).inputValue(),'nombre_propietario');
    assert.ok(await page.locator('#masterOnboardingFile').evaluate(el=>el.files.length===1));
    await page.locator('[data-onboarding-new-group]').click();
    await page.locator('#onboardingGroupForm [name="nombre"]').fill('General prueba '+suffix);
    await page.locator('#onboardingGroupForm button[type="submit"]').click();
    await page.locator('#onboardingGroupForm [role="alert"]').filter({hasText:'Ya existe'}).waitFor();
    await page.locator('#onboardingGroupCancel').click();
    await page.locator('#masterOnboardingPreview').click();
