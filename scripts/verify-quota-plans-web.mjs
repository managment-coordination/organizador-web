...
assert(source&&path.basename(path.dirname(source)).startsWith('organizador-quota-plans-'));
...
const seeded=spawnSync(python,['-',source,db],{encoding:'utf8',env:{...process.env,PYTHONPATH:path.join(root,'server')},input:`
...
const app=spawn(process.execPath,[path.join(root,'server/index.js')],{cwd:root,env:{...process.env,PYTHON_BIN:python,DATABASE_PATH:db,DATA_DIR:path.join(out,'data'),HOST:'127.0.0.1',PORT:String(port),AI_PROVIDER:'local',ERP4_BANKING_ENABLED:'0',ERP4_LIVE_BANKING_ENABLED:'0'},stdio:['ignore','pipe','pipe']});
...
try{
 ...
 for(let i=0;i<100;i++){try{if((await fetch(base+'/health')).ok)break;}catch{}if(app.exitCode!==null)throw Error(log);await delay(200);}
 ...
 await click('plan-new');await fill('name','Plan navegador');await fill('concept','Prueba de concepto');await page.locator('[name=exercise_id]').selectOption({label:'2028'});
 ...
 const row=page.getByRole('row').filter({hasText:'Plan navegador'});await row.getByRole('button',{name:'Desactivar',exact:true}).click();await fill('effective_from','2028-01-01');await review();await confirm();
 await click('period-emission');await page.locator('[name=exercise_id]').selectOption({label:'2028'});await fill('period_from','2028-07-01');await fill('issued_on','2028-07-03');await fill('due_on','2028-07-10');await review();
...
finally{await browser?.close();app.kill();await new Promise(r=>{if(app.exitCode!==null)r();else app.once('exit',r);});}
