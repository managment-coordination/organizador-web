...
from .receivables_projection import receipt_balance, collection_balance, credit_balance, responsibility_balance
...
class ReceivablesQueries:
    ...
    def debt_summary(self,session,query):
        def op(conn,q):
            ...
            for row in conn.execute('SELECT * FROM erp_saldos_apertura WHERE id_comunidad=? AND registered_at<=?',(q.community_id,f['known_at'])):
                ...
                amount=row['amount_cents'];opening_debt+=max(0,amount);opening_credit+=max(0,-amount)
    ...
    def receipt_timeline(self,session,query):
        def op(conn,q):
            ...
            for table in ('erp_imputaciones','erp_credito_aplicaciones','erp_rectificaciones','erp_recibo_gestion_eventos','erp_reasignaciones_obligacion'):
