...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .masterPropertyFlags { display:flex; gap:6px; flex-wrap:wrap; margin-top:6px; }
...
<body>
  ...
  <script>
    ...
    function masterPropertyLifecycleHtml(row) {
      const state=safe(row.estado||'activa').toLowerCase();
      if(state==='baja')return '<div class="masterQualityNotice"><strong>Propiedad dada de baja</strong><p>'+html(row.fecha_baja?'Desde '+row.fecha_baja:'Sin fecha indicada')+(row.motivo_baja?' · '+html(row.motivo_baja):'')+'</p><button id="masterReactivateProperty">Reactivar propiedad</button></div>';
      if(state==='preparacion'||state==='inactiva')return '<div class="masterQualityNotice"><strong>'+(state==='preparacion'?'Pendiente de activacion':'Propiedad inactiva')+'</strong><p>No participa en la operativa ordinaria mientras mantenga este estado.</p><button id="masterReactivateProperty">Activar propiedad</button></div>';
      return '<details class="masterPropertyControl"><summary>Gestionar baja de la propiedad</summary><form id="masterPropertyStatusForm" class="masterForm masterSection" data-target-status="baja"><div class="masterFormGrid"><label>Fecha de baja<input name="fecha_baja" type="date" required></label><label class="masterWide">Motivo<input name="motivo_baja" required></label></div><button>Confirmar baja</button></form></details>';
    }

    function masterPropertyQualityHtml(row) {
      const quality=safe(row.calidad_dato||'observada').toLowerCase();
      if(quality==='validada')return '';
      const title=quality==='pendiente_revision'?'Dato con incidencia pendiente':'Dato importado pendiente de validacion';
      const explanation=quality==='pendiente_revision'?'Revisa la informacion y resuelve el conflicto antes de validarla.':'La propiedad se conserva tal como consta en la fuente, pero aun no ha sido confirmada manualmente.';
      return '<div class="masterQualityNotice"><strong>'+title+'</strong><p>'+explanation+'</p><button id="masterValidateProperty">Confirmar datos como validados</button></div>';
    }
    ...
    function bindMasterDataPanel() {
      ...
      bindSubmit('#masterPropertyForm',async form=>{const data=formObject(form);const id=Number(data.id_propiedad||0),version=Number(data.version||0);delete data.version;if(!id){delete data.id_propiedad;data.estado='activa';data.calidad_dato='validada';}const result=await erpCommand('erp1.property.save',data,id?version:null);await loadMasterData();await selectMasterProperty(result.entity.id_propiedad);});
      bindSubmit('#masterPropertyStatusForm',async form=>{const data=formObject(form);data.id_propiedad=masterData.property.id_propiedad;data.estado=form.dataset.targetStatus;const result=await erpCommand('erp1.property.save',data,masterData.property.version);await loadMasterData();await selectMasterProperty(result.entity.id_propiedad);});
      root.querySelector('#masterReactivateProperty')?.addEventListener('click',async()=>{if(!confirm('Reactivar esta propiedad?'))return;try{const result=await erpCommand('erp1.property.save',{id_propiedad:masterData.property.id_propiedad,estado:'activa',fecha_baja:'',motivo_baja:''},masterData.property.version);await loadMasterData();await selectMasterProperty(result.entity.id_propiedad);}catch(error){alert(error.message);}});
      root.querySelector('#masterValidateProperty')?.addEventListener('click',async()=>{if(!confirm('Confirmas que has revisado los datos de esta propiedad?'))return;try{const result=await erpCommand('erp1.property.save',{id_propiedad:masterData.property.id_propiedad,calidad_dato:'validada'},masterData.property.version);await loadMasterData();await selectMasterProperty(result.entity.id_propiedad);}catch(error){alert(error.message);}});
